...
from mne.minimum_norm._eloreta import _compute_eloreta
...
from mne.io.pick import channel_type, pick_info, pick_types, pick_channels
from mne.cov import (compute_whitener, _read_cov, _write_cov, Covariance,
                   prepare_noise_cov)
...
from mne.forward.forward import write_forward_meas_info
from mne.source_space import (_read_source_spaces_from_tree, _get_src_nn,
...
from mne.utils import (check_fname, logger, verbose, warn, _validate_type,
...
def compute_kernel(epochs, inverse_operator, lambda2, method='dSPM', 
                   label=None, nave=1, pick_ori=None, prepared=False, 
                   method_params=None, use_cps=True):
    _check_option('method', method, INVERSE_METHODS)
    _check_ori(pick_ori, inverse_operator['source_ori'],
               inverse_operator['src'])
    ...
    return K, noise_norm, vertno, source_nn, sel

def _apply_inverse_epochs_gen(epochs, inverse_operator, K, noise_norm, vertno, 
                              source_nn, sel, pick_ori=None):
...
def apply_inverse_epochs(epochs, inverse_operator, lambda2, method="dSPM",
    ...
    apply_inverse : Apply inverse operator to evoked object.
    """
    K, noise_norm, vertno, source_nn, sel = compute_kernel(
    ...
    stcs = _apply_inverse_epochs_gen(
        epochs, inverse_operator, K, noise_norm, vertno, source_nn, sel, 
        pick_ori=pick_ori)
